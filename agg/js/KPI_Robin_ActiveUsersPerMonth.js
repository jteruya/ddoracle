...
activeuserspermonth_options = {
  chart: {
    renderTo: 'activeuserspermonth',
    type: 'column',
    zoomType: 'x',
    height: 500,
    // width: 1000,
  ...
  tooltip: {
    formatter: function () {
    var s = '<b>' + Highcharts.dateFormat('%b %Y', this.x) + '</b>: <br>' + Highcharts.numberFormat(this.y, 0, '', ',') + ' Active Users';
...
$.get('csv/KPI_Robin_ActiveUsersPerMonth.csv', function(data) {
  ...
  $.each(lines, function(lineNo, line) {
    ...
    if (lineNo == linecnt) {
      console.log('End');
      activeuserspermonth_options.series.push(series)
      dummy = 1;
    }    
    else if (lineNo > 0) {
...
activeuserspermonth = new Highcharts.Chart(activeuserspermonth_options);
