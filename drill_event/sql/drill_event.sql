...
-- Old Metrics will no longer be supported for this reporting
/*
--oldMetrics
...
--newMetrics
SELECT a.ApplicationId, a.GlobalUserId, a.Tapped, a.Identifier, a.Metadata, a.Url AS URL, a.ItemId, a.ListId
FROM (
        --Menu Item Actions
        SELECT ApplicationId, GlobalUserId, Created, Tapped, Identifier, Metadata, Url, ItemId, ListId FROM Report.NewMetrics_Actions_MenuItem

        UNION ALL
        --Chat Actions
        SELECT ApplicationId, GlobalUserId, Created, Tapped, Identifier, Metadata, CAST(NULL AS TEXT) AS Url, CAST(NULL AS INT) AS ItemId, CAST(NULL AS INT) AS ListId FROM Report.NewMetrics_Actions_Chat

        UNION ALL
        --Survey Actions
        SELECT ApplicationId, GlobalUserId, Created, Tapped, Identifier, Metadata, CAST(NULL AS TEXT) AS Url, ItemId, CAST(NULL AS INT) AS ListId FROM Report.NewMetrics_Actions_Survey

        UNION ALL
        --Feed Actions
        SELECT ApplicationId, GlobalUserId, Created, Tapped, Identifier, Metadata, CAST(NULL AS TEXT) AS Url, ItemId, CAST(NULL AS INT) AS ListId FROM Report.NewMetrics_Actions_Feed

        UNION ALL
        --Item Actions
        SELECT ApplicationId, GlobalUserId, Created, Tapped, Identifier, Metadata, CAST(NULL AS TEXT) AS Url, ItemId, ListId FROM Report.NewMetrics_Actions_Item

        UNION ALL
        --Profile Actions
        SELECT ApplicationId, GlobalUserId, Created, Tapped, Identifier, Metadata, CAST(NULL AS TEXT) AS Url, CAST(NULL AS INT) AS ItemId, CAST(NULL AS INT) AS ListId FROM Report.NewMetrics_Actions_Profile

        UNION ALL
        --Other Actions
        SELECT ApplicationId, GlobalUserId, Created, Tapped, Identifier, Metadata, CAST(NULL AS TEXT) AS Url, CAST(NULL AS INT) AS ItemId, CAST(NULL AS INT) AS ListId FROM Report.NewMetrics_Actions_Other

) a
JOIN AuthDB_Applications app ON a.ApplicationId = app.ApplicationId
WHERE a.ApplicationId IN (
        
        --Non-Test Events that started in the past 1 month
        SELECT ApplicationId FROM AuthDB_Applications
        WHERE StartDate >= CAST(EXTRACT(YEAR FROM CURRENT_DATE - INTERVAL'30 days')||'-'||EXTRACT(MONTH FROM CURRENT_DATE - INTERVAL'30 days')||'-01 00:00:00' AS TIMESTAMP) --Past 30 Days
        AND ApplicationId NOT IN (SELECT ApplicationId FROM EventCube.TestEvents)
) 
AND app.StartDate <= CURRENT_DATE --Only Events that have already started
AND app.StartDate >= CAST(EXTRACT(YEAR FROM CURRENT_DATE - INTERVAL'30 days')||'-'||EXTRACT(MONTH FROM CURRENT_DATE - INTERVAL'30 days')||'-01 00:00:00' AS TIMESTAMP) --Past 30 Days
AND a.Created >= CAST(EXTRACT(YEAR FROM CURRENT_DATE - INTERVAL'90 days')||'-'||EXTRACT(MONTH FROM CURRENT_DATE - INTERVAL'90 days')||'-01 00:00:00' AS TIMESTAMP) --Past 90 Days
AND app.ApplicationId NOT IN (SELECT ApplicationId FROM EventCube.TestEvents)
;
...
SELECT DISTINCT base.ApplicationId, app.Name, Tapped, COUNT(DISTINCT GlobalUserId) AS UsersTapped, COUNT(*) AS Taps, cnt.TotalTaps, 100 * CAST(COUNT(*) AS NUMERIC) / (cnt.TotalTaps) AS PCT_Taps
FROM (
        ...
        UNION ALL
        SELECT ApplicationId, GlobalUserId, Tapped FROM Event_Taps WHERE Tapped NOT IN ('Item','Menu - Item','Menu - Folder','Menu - List','Menu - Web View')
) base
JOIN AuthDB_Applications app ON base.ApplicationId = app.ApplicationId
JOIN (SELECT ApplicationId, COUNT(*) AS TotalTaps FROM Event_Taps GROUP BY 1) cnt ON base.ApplicationId = cnt.ApplicationId
GROUP BY base.ApplicationId, app.Name, Tapped, cnt.TotalTaps
ORDER BY base.ApplicationId, cnt.TotalTaps DESC;
