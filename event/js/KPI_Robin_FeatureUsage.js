...
featureusage_options = {
  chart: {
    renderTo: 'featureusage',
    type: 'spline',
    zoomType: 'x',
    height: 500,
    // width: 1000,
  ...
  title: {
    text: 'Events with specific Features Used - % by Month'
  },
  subtitle: {
...
$.get('csv/KPI_Robin_FeatureUsage.csv', function(data) {
  ...
  $.each(lines, function(lineNo, line) {
    ...
    if (lineNo == linecnt) {
      console.log('End');
      featureusage_options.series.push(series)
      dummy = 1;
    }    
    else if (lineNo > 0) {
...
featureusage = new Highcharts.Chart(featureusage_options);
