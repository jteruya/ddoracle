...
iosclosedloginfunneldevicesmonthly_options = {
  chart: {
    renderTo: 'iosclosedloginfunneldevicesmonthly',
    type: 'column',
    zoomType: 'x',
    height: 500,
    // width: 1000,
  ...
  title: {
    text: 'iOS Closed Bundle Login Funnel Devices per Month'
  },
  subtitle: {
...
$.get('csv/KPI_Robin_iOSClosedLoginFunnelDevicesMonthly.csv', function(data) {
  ...
  $.each(lines, function(lineNo, line) {
    ...
    if (lineNo == linecnt) {
      console.log('End');
      iosclosedloginfunneldevicesmonthly_options.series.push(series)
      dummy = 1;
    }    
    else if (lineNo > 0) {
...
iosclosedloginfunneldevicesmonthly = new Highcharts.Chart(iosclosedloginfunneldevicesmonthly_options);
