...
iosloginfunnelenterpasswordweekly_options = {
  chart: {
    renderTo: 'iosloginfunnelenterpasswordweekly',
    type: 'column',
    zoomType: 'x',
    height: 500,
    // width: 1000,
...
$.get('csv/KPI_Robin_iOSLoginFunnelWeekly.csv', function(data) {
  ...
  $.each(lines, function(lineNo, line) {
    ...
    if (lineNo == linecnt) {
      console.log('End');
      iosloginfunnelenterpasswordweekly_options.series.push(series)
      dummy = 1;
    }    
    else if (lineNo > 0) {
      ...
      if (items[0] == prev_name) {
        ...
        series.data.push([Date.UTC(year,month,day),parseFloat(items[4])])
...
iosloginfunnelenterpasswordweekly = new Highcharts.Chart(iosloginfunnelenterpasswordweekly_options);
