...
androidclosedloginfunneldevicesmonthly_options = {
  chart: {
    renderTo: 'androidclosedloginfunneldevicesmonthly',
    type: 'column',
    zoomType: 'x',
    height: 500,
    // width: 1000,
  ...
  title: {
    text: 'Android Closed Bundle Login Funnel Devices per Month'
  },
  subtitle: {
...
$.get('csv/KPI_Robin_AndroidClosedLoginFunnelDevicesMonthly.csv', function(data) {
  ...
  $.each(lines, function(lineNo, line) {
    ...
    if (lineNo == linecnt) {
      console.log('End');
      androidclosedloginfunneldevicesmonthly_options.series.push(series)
      dummy = 1;
    }    
    else if (lineNo > 0) {
...
androidclosedloginfunneldevicesmonthly = new Highcharts.Chart(androidclosedloginfunneldevicesmonthly_options);
