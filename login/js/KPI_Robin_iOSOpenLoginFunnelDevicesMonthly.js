...
iosopenloginfunneldevicesmonthly_options = {
  chart: {
    renderTo: 'iosopenloginfunneldevicesmonthly',
    type: 'column',
    zoomType: 'x',
    height: 500,
    // width: 1000,
  ...
  title: {
    text: 'iOS Open Bundle Login Funnel Devices per Month'
  },
  subtitle: {
...
$.get('csv/KPI_Robin_iOSOpenLoginFunnelDevicesMonthly.csv', function(data) {
  ...
  $.each(lines, function(lineNo, line) {
    ...
    if (lineNo == linecnt) {
      console.log('End');
      iosopenloginfunneldevicesmonthly_options.series.push(series)
      dummy = 1;
    }    
    else if (lineNo > 0) {
...
iosopenloginfunneldevicesmonthly = new Highcharts.Chart(iosopenloginfunneldevicesmonthly_options);
