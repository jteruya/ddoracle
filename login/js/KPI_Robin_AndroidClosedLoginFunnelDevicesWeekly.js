...
androidclosedloginfunneldevicesweekly_options = {
  chart: {
    renderTo: 'androidclosedloginfunneldevicesweekly',
    type: 'column',
    zoomType: 'x',
    height: 500,
    // width: 1000,
  ...
  title: {
    text: 'Android Closed Bundle Login Funnel Devices per Week'
  },
  subtitle: {
...
$.get('csv/KPI_Robin_AndroidClosedLoginFunnelDevicesWeekly.csv', function(data) {
  ...
  $.each(lines, function(lineNo, line) {
    ...
    if (lineNo == linecnt) {
      console.log('End');
      androidclosedloginfunneldevicesweekly_options.series.push(series)
      dummy = 1;
    }    
    else if (lineNo > 0) {
...
androidclosedloginfunneldevicesweekly = new Highcharts.Chart(androidclosedloginfunneldevicesweekly_options);
