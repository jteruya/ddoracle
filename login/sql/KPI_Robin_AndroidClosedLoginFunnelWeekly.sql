...
select 'EnterEmail' as "Step Name"
     ...
     , case
         when count(*) > 0 then cast((count(case when checklist.enterEmailMinDate is not null or checklist.accountPickerMinDate is not null then 1 else null end)::decimal(8,2) - count(case when checklist.enterPasswordMinDate is not null then 1 else null end)::decimal(8,2))/count(*)::decimal(8,2) * 100 as decimal(5,2))
         else null
