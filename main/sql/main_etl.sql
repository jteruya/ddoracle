...
WHERE (CAST(EXTRACT(WEEK FROM CURRENT_DATE) AS INT) - CAST(EXTRACT(WEEK FROM b.StartDate) AS INT) < 8 AND CAST(EXTRACT(WEEK FROM CURRENT_DATE) AS INT) - CAST(EXTRACT(WEEK FROM b.StartDate) AS INT) >= 1 AND EXTRACT(YEAR FROM CURRENT_DATE) = EXTRACT(YEAR FROM b.StartDate)) --Within 8 weeks, same year
