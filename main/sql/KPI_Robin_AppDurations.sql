...
SELECT YYYY_WW, UserId, GlobalUserId, SUM(Duration_Seconds) AS Total_Duration_Seconds, ROUND(CAST(SUM(Duration_Seconds) AS Numeric) / 60,2) AS Total_Duration_Minutes FROM dashboard.App_Durations_Weekly t GROUP BY 1,2,3
) t GROUP BY 1;
